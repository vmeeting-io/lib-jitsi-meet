...
export default class ChatRoom extends Listenable {
    ...
    discoRoomInfo() {
        ...
        this.connection.sendIQ(getInfo, result => {
            ...
            const timerEndTime = parseInt($(result).find('>query>x[type="result"]>field[var="muc#roominfo_timer_end_time"]>value').text(), 10);
            const timerInitiator = $(result).find('>query>x[type="result"]>field[var="muc#roominfo_timer_initiator"]>value').text();
            if (timerEndTime) {
                try {
            ...
            // Timer: End
            const timeRemained = $(result).find('>query>x[type="result"]>field[var="muc#roominfo_timeremained"]>value');
            ...
            const faceDetect = $(result).find('>query>x[type="result"]>field[var="muc#roominfo_facedetect"]>value');
            ...
            const sttEnabled = $(result).find('>query>x[type="result"]>field[var="muc#roominfo_sttenabled"]>value');
    ...
    onPresence(pres) {
        ...
        if (from === this.myroomjid) {
            const newRole = member.affiliation === 'owner' ? member.role : member.role; // ternary option was initially 'none';
            ...
            if (member.isFocus) {
                ...
                this.eventEmitter.emit(XMPPEvents.MUC_MEMBER_JOINED, from, member.nick, member.role, member.isHiddenDomain, member.statsID, member.status, member.identity, member.botType, member.jid, member.features, member.bDate, member.hatOn, member.isReplaceParticipant);
 ...
 */
    ...
    setSubject(subject) {
        ...
        this.connection.send(msg);
        if ((_a = this.breakoutRooms) === null || _a === void 0 ? void 0 : _a.isBreakoutRoom()) {
     ...
     */
    onMessage(msg, from) {
        const nick = $(msg).find('>nick[xmlns="http://jabber.org/protocol/nick"]')
            .text()
            || Strophe.getResourceFromJid(from);
        ...
        if (txt) {
            if (type === 'chat') {
                this.eventEmitter.emit(XMPPEvents.PRIVATE_MESSAGE_RECEIVED, from, nick, txt, this.myroomjid, stamp);
            }
    ...
    muteParticipant(jid, mute, mediaType) {
        ...
        const iqToJid = $iq({ to: jid,
            type: 'set' })
            .c('mute', {
            xmlns: `http://jitsi.org/jitmeet/${mediaType}`,
            jid
        })
            .t((mute === null || mute === void 0 ? void 0 : mute.toString()) || 'true')
            .up();
        this.connection.sendIQ(iqToJid, result => logger.log('set mute', result), error => logger.log('set mute error', error));
    }
    /**
     * TODO: Document
     * @param iq
    ...
    onMute(iq) {
        ...
        if (mute.length) {
            this.eventEmitter.emit(XMPPEvents.AUDIO_MUTED_BY_FOCUS, from, mute.text() === 'true');
        }
        else {
    ...
    onMuteVideo(iq) {
        ...
        if (mute.length && mute.text() === 'true') {
            this.eventEmitter.emit(XMPPEvents.VIDEO_MUTED_BY_FOCUS, from, mute.text() === 'true');
        }
        else {
            // XXX Why do we support anything but muting? Why do we encode the
            // value in the text of the element? Why do we use a separate XML
